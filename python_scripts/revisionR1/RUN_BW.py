...
def _get_proposal(ind_idx):
    ...
    proposal = MultivariateNormal(loc=torch.Tensor(ann_res.ann_res.x), 
...
for cur_ind_idx in range(0, 36):
    ...
    # here I use a flat prior but my proposal is very informative
    prior = MultivariateNormal(loc=torch.zeros(3), 
                           covariance_matrix=torch.eye(3)*(paras.prior_sd)**2)
    proposal = _get_proposal(cur_ind_idx)
